...
def worker(path, opt):
    ...
    for x in h_space:
        for y in w_space:
            index += 1
            cropped_img = img[x:x + crop_size, y:y + crop_size, ...]  # x:x+a 裁剪图片，长度是a
            cropped_img = np.ascontiguousarray(cropped_img)  # 使经过裁剪之后的数组连续，加快运算速度
            cv2.imwrite(
                osp.join(opt['save_folder'],
                         f'{img_name}_s{index:03d}{extension}'), cropped_img,  # 支持{}内的变量
