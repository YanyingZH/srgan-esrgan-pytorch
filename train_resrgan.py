...
from models import SRRDBNet, RERaDiscriminator
...
netG = SRRDBNet(opt['in_channels'],
               opt['ngf'],
               opt['num_blocks'],
               opt['num_grow'],
               opt['gaussian_noise']).to(device)
netD = RERaDiscriminator(opt['in_channels'], opt['ndf']).to(device)

# 网络初始化权重参数
# netG.load_state_dict(osp.join('experiments', 'pretrained_models', 'SRRDBNet_final.pth'))
